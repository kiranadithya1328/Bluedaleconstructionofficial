...
function setupPremium3DExperience() {
  document.body.classList.add('premium-shell');
  createPremiumAtmosphere();
  setupScrollProgress();
  setupPremiumPageTransitions();
  setup3DTilt();
  setupMagneticButtons();
}

function refreshPremiumInteractions(root = document) {
  setup3DTilt(root);
  setupMagneticButtons(root);
  setupPremiumPageTransitions();
}

function createPremiumAtmosphere() {
...
function setupPremiumPageTransitions() {
  ...
  document.querySelectorAll('a[href]').forEach(link => {
    ...
    if (!href || href.startsWith('#') || href.startsWith('mailto:') || href.startsWith('tel:') || href.startsWith('https://wa.me') || target === '_blank') return;
...
function setup3DTilt(root = document) {
  const selectors = [
    '.service-card',
    '.portfolio-card',
    '.value-card',
...
function setupMagneticButtons(root = document) {
  root.querySelectorAll('.btn, .filter-btn, .social-icon').forEach(button => {
...
function getFallbackWorks() {
  return [
    { id: "w1", name: "The Horizon Villa",   year: 2024, category: "residential", location: "Green Valley Suburbs", description: "A premium double-story custom residential villa built with modern glass facades and wooden panel cladding.",                       imageUrl: "work_villa.png" },
    { id: "w2", name: "Apex Business Hub",   year: 2023, category: "commercial",  location: "Tech Park Central",    description: "A modern state-of-the-art office corporate complex featuring sleek structural glass curtain walls and steel framings.",          imageUrl: "work_commercial.png" },
    { id: "w3", name: "Skyline Residency",   year: 2025, category: "residential", location: "North Ridge Blvd",     description: "A luxury mid-rise apartment complex featuring private botanical balconies and architectural lighting.",                           imageUrl: "work_apartment.png" },
    { id: "w4", name: "Maple Wood Estate",   year: 2024, category: "renovations", location: "Maple Wood Suburbs",   description: "Complete exterior and interior makeover of a suburban family residence with structural frame updates and modern cladding.",        imageUrl: "work_residence.png" }
  ];
}
...
function loadHomePageData() {
  ...
  getWorksData(works => {
    ...
    works.slice(0, 2).forEach((w, i) => {
      ...
      card.innerHTML = `
        <img src="${w.imageUrl || 'work_villa.png'}" alt="${w.name}" class="portfolio-img" onerror="this.src='work_villa.png'">
    ...
    initScrollReveals();
    refreshPremiumInteractions(grid);
  });
}
